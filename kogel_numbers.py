...
def start():
    ...
    if filter_digit == -1:
        ...
        main_filtered(file_str, count, filter_digit)
    end_time = time.time()
    ...
    while index <= count:  # Check all digits in file
        ...
        while shift < length:  # Check all possible shifts
            ...
            while digit < length:  # Check all digits of the number
                if file[index + digit] != str(index_with_shift)[digit]:
                    break
                if digit == length - 1:
...
def main_filtered(file: str, count: int, filter_digit: int):
    index = 0
    bar_number = count // 531
    while index <= count:
        if index % bar_number == 0:
